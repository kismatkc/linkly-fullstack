  ...
  return (
    <header className="flex justify-between pt-8 px-4">
      ...
      <div className="flex-row-center gap-x-3">
        <ThemeToggle />
        <button className="flex-row-center gap-x-2 bg-brand-grey-dark text-white rounded-full px-5 py-2" onClick={()=>{
          signOut()
        }}>
          <Avatar>
            {session?.user?.image ? (
              ...
              <AvatarFallback className="text-brand-grey-dark font-extrabold dark:text-brand-white">
                {
                  //@ts-ignore
